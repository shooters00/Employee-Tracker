...
const getRole = async () => {
    ...
    const getTheRole = async (answer) => {
        console.log(answer);
        const RoleID = await db.promise().query(`SELECT id FROM roles WHERE name=?`, answer.role_id);
    ...
    const getEmployee = async () => {
        ...
        return new Promise(async (resolve, reject) => {
            const roleNames = await db.promise().query(`SELECT title FROM roles`);
            ...
            .then(async (answer) => {
                const depID = await getTheRole(answer);
                return db.promise().query(`INSERT INTO employees (title, salary, department_id) VALUES (?, ?, ?)`, [answer.title, answer.salary, depID]) 
            }).then(() => {
                console.log("\n");
                console.log("Role added.");
                resolve();

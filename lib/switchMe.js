...
const getRole = async () => {
    ...
    const updateEmployeeRole = async () => {
        ...
        return new Promise(async (resolve, reject) => {
            const employeeNames = await db.promise().query(`SELECT id, first_name, last_name FROM employees`);
            const formattedEmployeeNames = employeeNames[0].map((item) => {
                return {
                    name: item.first_name + " " + item.last_name, 
                    value: item.id
                }
            ...
            const employeeQuest = [
                {
                    ...
                    choices: formattedEmployeeNames
                },
                {
                    type: 'list',
                    name: 'role',
            ...
            .then(async ({employee, role}) => {
                //const roleID = await getTheRole(answer);
                //const managerID = await getTheManager(answer);
                return db.promise().query(`UPDATE employees SET role_id=? WHERE id=?`, [role, employee]) 
                //return db.promise().query(`UPDATE employees SET role_id = ${role} WHERE id=${employee}`); 
            }).then(() => {
                console.log("\n");
